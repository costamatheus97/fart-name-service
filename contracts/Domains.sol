...
contract Domains is ERC721URIStorage {
  ...
  constructor(string memory _tld) payable ERC721("Fart Name Service", "FNS") {
    tld = _tld;
      ...
      bytes(
        string(
          abi.encodePacked(
            '{"name": "',
            _name,
            '", "description": "A domain on the Fart name service", "image": "data:image/svg+xml;base64,',
  ...
  function transferFunds(string calldata receiverName, uint256 amount) public payable returns (bool) {
        address receiver = getAddress(receiverName);
